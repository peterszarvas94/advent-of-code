...
impl Hand {
  ...
  fn get_combination(&self) -> Combination {
    ...
    // add logic to handle J as wildcard
    // jjjjj -> FiveOfKind
    ...
    let jack = counts_vec.iter().find(|(card, _)| *card == Card::Jack);
    match jack {
      Some((_, 4..=5)) => return Combination::FiveOfKind,
      Some((_, 3)) => match second {
        Some((_, 2)) => return Combination::FiveOfKind,
        _ => return Combination::FourOfKind,
      },
      Some((_, 2)) => {
        if let Some((_, 3)) = first {
          return Combination::FiveOfKind;
        }
        if let Some((_, 2)) = second {
          return Combination::FourOfKind;
        }
        return Combination::ThreeOfKind;
      }
      Some((_, 1)) => match first {
        Some((_, 4)) => return Combination::FiveOfKind,
        Some((_, 3)) => return Combination::FourOfKind,
        Some((_, 2)) => match second {
          Some((_, 2)) => return Combination::FullHouse,
          Some((_, 1)) => return Combination::ThreeOfKind,
          _ => {}
        },
        Some((_, 1)) => return Combination::OnePair,
        _ => {}
      },
      _ => {}
    }

    // if not jack, then check for other combinations
...
fn main() {
  ...
  for (i, hand) in hands.iter().enumerate() {
    println!("{}, {:?}, {:?}", i + 1, hand.get_combination(), hand);
    let factor = i + 1;
